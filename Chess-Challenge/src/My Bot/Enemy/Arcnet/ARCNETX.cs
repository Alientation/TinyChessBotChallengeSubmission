...
public class ARCNETX : IChessBot
{
  /*
  TODO:
  PSTS
  ...
  */
  ...
  static int[] pawnTableMiddleGame = {
      0,   0,   0,   0,   0,   0,  0,   0,
  ...
  static int[] knightTableMiddleGame = {
  ...
  static int[] bishopTableMiddleGame = {
  ...
  static int[] rookTableMiddleGame = {
  ...
  static int[] queenTableMiddleGame = {
  ...
  static int[] kingTableMiddleGame = {
  ...
  int[][] pieceTablesMiddleGame = {
    null,
    pawnTableMiddleGame,
    knightTableMiddleGame,
    bishopTableMiddleGame,
    rookTableMiddleGame,
    queenTableMiddleGame,
    kingTableMiddleGame,
  };

  int[][] pieceTablesEndGame = {
    null,
    pawnTableEndGame,
    knightTableEndGame,
    bishopTableEndGame,
  ...
  class State
  {
    ...
    public State(Move move, ARCNETX me)
    {
      Me = me;
  ...
  int pieceWorth(int pieceTypeIndex, Piece piece, float middleGame) => pieceValues[pieceTypeIndex]
  + (int)(pieceTablesMiddleGame[pieceTypeIndex][piece.Square.Index] * middleGame)
  + (int)(pieceTablesMiddleGame[pieceTypeIndex][piece.Square.Index] * (1f - middleGame));
  ...
  int Evaluate()
  {
    ...
    float middleGame = 1f - _board.PlyCount / 80f;

    for (int typeIndex = 1; typeIndex < 7; typeIndex++)
    {
      materialEvaluation += _board.GetPieceList((PieceType)typeIndex, true).Sum(piece => pieceWorth(typeIndex, piece, middleGame));
      materialEvaluation -= _board.GetPieceList((PieceType)typeIndex, false).Sum(piece => pieceWorth(typeIndex, piece, middleGame));
    }
  ...
  public Move Think(Board board, Timer timer)
  {
    ...
    for (int targetDepth = tree.Depth + 1; tree.ChildStates == null || timer.MillisecondsElapsedThisTurn * 1.4 < timer.MillisecondsRemaining / 60; targetDepth++)
    {
